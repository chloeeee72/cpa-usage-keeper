...
export async function updateUsageIdentityBalanceSession(id: string, session: string | null): Promise<UsageIdentity> {
  const response = await apiFetch(apiPath(`/usage/identities/${encodeURIComponent(id)}/balance-session`), {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function fetchBalanceSummary(options?: { identityId?: string; signal?: AbortSignal }): Promise<BalanceQueryResponse> {
  const response = await apiFetch(apiPath('/usage/balance/query'), {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: options?.identityId ? JSON.stringify({ identity_id: options.identityId }) : '{}',
    signal: options?.signal,
  })
  if (!response.ok) {
    await parseApiError(response, `Failed to load balance summary: ${response.status}`)
  }
  return response.json()
}
