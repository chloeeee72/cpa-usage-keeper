...
describe('Credential section styles', () => {
  ...
  it('lets Auth Files quota bars wrap before their blocks overlap', () => {
    ...
    expect(credentialStyles).toMatch(/\.credentialHealthPanel\s*\{[\s\S]*?grid-template-columns:\s*max-content minmax\(0, 1fr\);/)
    expect(credentialStyles).toMatch(/\.credentialHealthPanel\s*\{[\s\S]*?column-gap:\s*16px;/)
